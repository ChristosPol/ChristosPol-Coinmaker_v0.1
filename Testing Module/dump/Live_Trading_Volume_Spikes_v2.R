...
repeat {
  ...
  # 2. Add Indicators
  df[, c(paste0("Volume_EMA", "_", Volume_EMA),
         "pattern",
         "exit_value",
         "candle_type",
         "exit_condition",
  ...
  df[, c(paste0("Volume_EMA", "_", Volume_EMA)) := list(EMA(volume, n = Volume_EMA) ) ]
  ...
  df$crossover[df$volume > get(paste0("Volume_EMA", "_", Volume_EMA), df)  ] <- "volume_higher"
  df$crossover[df$volume <= get(paste0("Volume_EMA", "_", Volume_EMA), df)  ] <- "volume_lower"  
  
  df$pattern[nrow(df)] <- df$candle_type[nrow(df)] == "bullish" & df$candle_type[nrow(df) -1] == "bearish"
  ...
  if (nrow(da) > 1) {
    ...
    # BUY Condition ------------------------------------------------------------
    if ((is.na(da$action[nrow(da) - 1]) |  da$action[nrow(da) - 1] %in% c("sell", "no action")) & da$crossover[nrow(da)] == "volume_higher" & da$pattern[nrow(da)] == 1  ){  
